...
- **Modern UI & Full Dark Mode**:
  - **Compact Material3 Floating Pill Navigation Bar**: Ultra-sleek, compact floating card container (max width 300dp) with 36dp rounded corners and 12dp elevation shadow. The active screen (which displays its header title at top) shows a filled icon pill with no text label, while inactive tabs display crisp text labels underneath visible in both Light and Dark modes.
